        ...
        case .p2sh:
            if let redeemScript = output.redeemScript {
                if let signatureScriptFunction = output.signatureScriptFunction {
                    ...
                    sigScriptLength = TransactionSizeCalculator.signatureLength + TransactionSizeCalculator.pubKeyLength + OpCode_Local_Usage.push(redeemScript).count
                }
            } else {
                sigScriptLength = 0
            }
...
extension TransactionSizeCalculator: ITransactionSizeCalculator {
    ...
    public func transactionSize(previousOutputs: [Output], outputScriptTypes: [ScriptType_Local_Usage]) -> Int {      // in real bytes upped to int
        transactionSize(previousOutputs: previousOutputs, outputScriptTypes: outputScriptTypes, pluginDataOutputSize: 0)
    }

    public func transactionSize(previousOutputs: [Output], outputScriptTypes: [ScriptType_Local_Usage], pluginDataOutputSize: Int) -> Int {      // in real bytes upped to int
        var segWit = false
        var inputWeight = 0
